...
import { UserOut, UserCreate, UserUpdate, ChangePassword, AssignUserToFarm, UserFarm, UserListItem } from '../models/user.model';
...
export class UserService {
  ...
  getUsers(granjaId?: number, status?: string, search?: string): Observable<UserListItem[]> {
    let params = new HttpParams();
    if (granjaId) params = params.set('granja_id', granjaId);
    if (status) params = params.set('status', status);
    if (search) params = params.set('search', search);
    
    return this.http.get<UserListItem[]>(this.API_URL, { params });
  }
